...
namespace DSP_Battle
{
    class UIBattleStatistics
    {
        ...
        //护盾承受伤害
        public static void RegisterShieldTakeDamage(int damage)
        {
        ...
        public static bool CargoPathUpdatePrePatch(ref CargoPath __instance)
        {
            byte[] obj;
                ...
                lock (obj)
                {
                    ...
                    lock (obj2)
                    {
                        ...
                        if (__instance.buffer[num] == 250)
                        {
                            ...
                            {
                                if (__instance.outputPath.TryInsertCargoNoSqueeze(__instance.outputIndex + 5, cargoId))
                                {
                                    Array.Clear(__instance.buffer, num - 4, 10);
                                    __instance.updateLen = __instance.bufferLength;
                                }
                                else if (__instance.outputPath.TryInsertCargoNoSqueeze(__instance.outputIndex, cargoId))
                                {
                                    Array.Clear(__instance.buffer, num - 4, 10);
                                    __instance.updateLen = __instance.bufferLength;
                                }
                            }
                            else if (__instance.outputPath.TryInsertCargo(__instance.outputIndex + 5, cargoId))
                            {
                                Array.Clear(__instance.buffer, num - 4, 10);
                                __instance.updateLen = __instance.bufferLength;
                            }
            ...
            lock (obj)
            {
                ...
                if (__instance.updateLen != 0)
                {
                    ...
                    for (int i = __instance.chunkCount - 1; i >= 0; i--)
                    {
                        ...
                        int num5 = __instance.chunks[i * 3 + 2];
                        if (num4 < num3)
                        {
